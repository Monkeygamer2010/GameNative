...
extern "C" JNIEXPORT jlong JNICALL
Java_com_winlator_core_GPUInformation_getGPUDriverVersion(JNIEnv*, jclass)
{
    static jlong cached{};                // memoise result
    ...
    EGLConfig cfg;
    EGLint cfgCnt;
    eglChooseConfig(dpy, cfgAttr, &cfg, 1, &cfgCnt);

    const EGLint ctxAttr[] = { EGL_CONTEXT_CLIENT_VERSION, 2, EGL_NONE };
    EGLContext ctx = eglCreateContext(dpy, cfg, EGL_NO_CONTEXT, ctxAttr);
    eglMakeCurrent(dpy, EGL_NO_SURFACE, EGL_NO_SURFACE, ctx);

    /* grab first 8-plus-digit run in GL_VERSION -------------------------- */
    if (const char* ver = reinterpret_cast<const char*>(glGetString(GL_VERSION)); ver) {
        for (const char* p = ver; *p; ++p) {
            if (*p >= '0' && *p <= '9') {
                const char* s = p;
                while (*p >= '0' && *p <= '9') ++p;
                if (p - s >= 8) {          // Qualcomm build numbers are long
                    char buf[24];
                    size_t n = (p - s < sizeof(buf) - 1) ? (p - s) : sizeof(buf) - 1;
                    memcpy(buf, s, n);
                    buf[n] = '\0';
                    cached = static_cast<jlong>(strtoull(buf, nullptr, 10));
                    break;
                }
            }
        }
    ...
    eglTerminate(dpy);
    return cached;                         // 0 if nothing found
}
